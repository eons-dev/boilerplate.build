...
class boilerplate(Builder):
	...
	# Create /build folder
	def SetupBuild(this):
		buildFile = this.CreateFile('./build/build.yaml')
		buildFile.write(f'''\
...
''')
		...
		githubBuildFile = this.CreateFile('./build/config/github.yaml')
...
''')
		...
		localBuildFile = this.CreateFile('./build/config/local.yaml')
		localBuildFile.write('''#YOUR CODE GOES HERE
''')
		localBuildFile.close()

		scheduleBuildFile = this.CreateFile('./build/config/schedule.yaml')
